...
class Player(BasePlayer):
    ...
    instruction_quiz_5 = models.StringField(
        choices=[["correct", r"Proposer: \(\mathit{payoff}_P = 20 - S + R\); Responder: \(\mathit{payoff}_R = S M - R\)."],
                 ["swapped", r"Proposer: \(\mathit{payoff}_P = S M - R\); Responder: \(\mathit{payoff}_R = 20 - S + R\)."],
                 ["same", r"Both receive \(\mathit{payoff} = S M - R\)."]],
    ...
    instruction_quiz_7 = models.StringField(
        choices=[["quadratic", r"The report and realized multiplier determine a winning chance. The prize is paid when the random integer \(Y\) is no greater than that winning chance."],
        ...
        label="How can a selected probability report determine the belief prize?", widget=widgets.RadioSelect,
    )
...
def get_high_multiplier_probability(session):
    return config_float(session, "high_multiplier_probability", C.DEFAULT_HIGH_MULTIPLIER_PROBABILITY)
...
def creating_session(subsession: Subsession):
    ...
    if subsession.round_number == 1:
        ...
        # Picture exposure is a between-session treatment: every participant in
        # the session either sees pictures throughout or never sees pictures.
        subsession.session.vars["picture_condition"] = bool(
            subsession.session.config.get("picture_condition", False)
        )
    ...
    for group in subsession.get_groups():
        ...
        group.treatment_code = "picture" if picture else "anonymous"
        group.treatment_label = "Picture" if picture else "No picture"
...
class ProposerReceipt(Page):
    ...
    def vars_for_template(player: Player):
        ...
        return dict(**page_common_vars(player), partner=partner, partner_profile=profile_for(partner),
                    show_profile=treatment_picture(player), is_practice=player.is_practice_round,
...
class WaitForPostBelief(WaitPage):
    title_text = "Waiting"
    body_text = "Please wait while the proposer completes the second probability report."
...
class PartnerIdentification1(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 1 <= get_active_periods(player.session)
...
class PartnerIdentification2(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 2 <= get_active_periods(player.session)
...
class PartnerIdentification3(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 3 <= get_active_periods(player.session)
...
class PartnerIdentification4(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 4 <= get_active_periods(player.session)
...
class PartnerIdentification5(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 5 <= get_active_periods(player.session)
...
class PartnerIdentification6(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 6 <= get_active_periods(player.session)
...
class PartnerIdentification7(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 7 <= get_active_periods(player.session)
...
class PartnerIdentification8(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 8 <= get_active_periods(player.session)
...
class PartnerIdentification9(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 9 <= get_active_periods(player.session)
...
class PartnerIdentification10(Page):
    ...
    @staticmethod
    def is_displayed(player: Player):
        return show_end_demographic_survey(player) and treatment_picture(player) and 10 <= get_active_periods(player.session)
...
def partner_survey_values(player):
    final_player = final_active_player(player)
    values = []
    if not get_picture_condition(player.session):
...
def custom_export(players):
    base_headers = [
        ...
        "picture_condition", "high_multiplier_probability", "low_multiplier", "high_multiplier",
    ...
    for player in players:
        ...
        base_values = [
            ...
            player.session.vars.get("unique_cross_role_pairs"), group.treatment_picture,
